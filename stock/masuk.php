<?php 
	...
	if(isset($_POST['addbrgmasuk']))
	{
		...
		$dt=mysqli_query($conn,"select * from stock_brg where id='$barang'");
		$data=mysqli_fetch_array($dt);
		$sisa=$data['stock']+$qty;
		$updatestock = mysqli_query($conn,"update stock_brg set stock='$sisa' where id='$barang'");
		$addlog = mysqli_query($conn,"insert into brg_masuk values('','$tanggal','$barang','$qty','$ket')");

		if($updatestock&&$addlog){
				echo " <div class='alert alert-success' align='center'>
					<strong>Berhasil!</strong> Data barang masuk berhasil disimpan.
				  </div>
				<meta http-equiv='refresh' content='1; url= masuk.php'/>  ";
				} else { echo "<div class='alert alert-warning' align='center'>
					<strong>Gagal!</strong> Data barang masuk gagal disimpan.
				  </div>
				 <meta http-equiv='refresh' content='1; url= masuk.php'/> ";
				}
                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="d-sm-flex justify-content-between align-items-center">
									<h2>Barang Masuk</h2>
									<button style="margin-bottom:20px" data-toggle="modal" data-target="#myModal" class="btn btn-success col-md-2"><span class="glyphicon glyphicon-plus"></span>Tambah Manual</button>
                                </div>
                                <div class="market-status-table mt-4">
                                    <div class="table-responsive">
										 <table id="dataTable3" class="table table-hover" style="width:100%"><thead class="thead-dark">
											...
											<?php 
											$brg=mysqli_query($conn,"SELECT * from brg_masuk b, stock_brg s where b.idbarang=s.id order by tgl DESC");
											$no=1;
									...
									<a href="barangmasuk.php" target="_blank" class="btn btn-info">Export Data</a>
                                </div>
                            </div>
                        </div>
                    </div>
						...
						<div class="modal-body">
							<form method="post">
								...
								<div class="form-group">
									...
									<?php
									$det=mysqli_query($conn,"select * from stock_brg order by nama ASC")or die(mysqli_error());
									while($d=mysqli_fetch_array($det)){
									?>
										<option value="<?php echo $d['id'] ?>"><?php echo $d['nama'] ?> <?php echo $d['jenis'] ?></option>
										<?php
								}
								?>
									</select>
							...
							<div class="modal-footer">
								...
								<input name="addbrgmasuk" type="submit" class="btn btn-primary" value="Simpan">
							</div>
						</form>
					</div>
				</div>
